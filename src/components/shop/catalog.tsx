  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="flex min-w-0 flex-col">
              <h1 className="font-serif text-xl font-bold leading-none text-primary md:text-2xl">
          ...
          {filteredProducts.map((product, index) => {
            ...
            return (
              <Card
                ...
                className="group cursor-pointer gap-0 overflow-hidden p-0 py-0 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
              >
                <div className="relative aspect-4/5 w-full overflow-hidden bg-muted">
                  {product.image_url ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-muted to-muted/40 text-5xl">
                      <span aria-hidden>🍕</span>
                    </div>
                  )}
                  <div className="pointer-events-none absolute inset-0 ring-1 ring-inset ring-foreground/5" />
                </div>
                <div className="flex min-h-26 flex-col justify-between gap-2 p-3 md:min-h-28 md:p-4">
                  <div className="flex flex-col gap-1">
                    <h3 className="line-clamp-1 font-serif text-base leading-tight font-bold text-foreground md:text-lg">
                      {product.name}
                    </h3>
                    {product.description ? (
                  ...
                  <div className="flex items-center justify-between gap-2">
                    {min !== null ? (
                      ...
                      <p className="text-xs text-muted-foreground">Sin tamaños</p>
                    )}
                    <span
                      aria-hidden
                      className="inline-flex size-9 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-sm transition-transform group-active:scale-95"
                    >
      ...
      >
        <SheetContent
          side="bottom"
          className="theme-heritage h-[92dvh] w-full max-w-none overflow-hidden p-0 sm:max-w-none md:inset-y-0 md:right-0 md:left-auto md:h-dvh md:w-[28rem] md:max-w-[28rem] md:border-l"
        >
