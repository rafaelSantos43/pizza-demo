...
export function OrderCard({ order, onSelect }: OrderCardProps) {
  ...
  const address = isPickup ? "Recoger en el local" : shortAddress(order.address);
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {isPickup ? (
            <span className="inline-flex items-center rounded-full bg-secondary px-2 py-0.5 text-[11px] font-semibold tracking-wider text-secondary-foreground uppercase">
              Recoger
            </span>
          ) : null}
        </div>
