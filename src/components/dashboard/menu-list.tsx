  ...
  return (
    <>
      ...
      <button
        type="button"
        onClick={() => setIsCreating(true)}
        className="fixed right-4 bottom-4 z-30 flex size-14 items-center justify-center rounded-full bg-success text-success-foreground shadow-lg transition-transform hover:scale-105 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring md:hidden"
        style={{
...
function EmptyState({ onCreate }: { onCreate: () => void }) {
  return (
    <Card className="flex flex-col items-center gap-3 border-dashed px-6 py-14 text-center">
      ...
      <Button onClick={onCreate} variant="success" className="mt-2 min-h-11">
        <Plus className="size-4" />
...
function ProductImage({ product }: { product: Product }) {
  if (!product.image_url) {
    return (
      <div
        className="flex size-14 shrink-0 items-center justify-center rounded-lg bg-secondary/50 text-xl ring-1 ring-inset ring-foreground/5"
        aria-hidden
      >
        🍕
      </div>
  ...
  return (
    <Image
      ...
      height={56}
      unoptimized
      className="size-14 shrink-0 rounded-lg object-cover object-center ring-1 ring-inset ring-foreground/5"
    />
  );
}
