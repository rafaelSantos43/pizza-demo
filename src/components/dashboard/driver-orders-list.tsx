...
export function DriverOrdersList({
  ...
  useEffect(() => {
    ...
    function notifyAssigned(orderId: string, status: string): void {
      ...
      // El mensaje depende del status con el que el pedido se volvió actuable:
      // ready = hay que recogerlo en el local y salir; on_the_way = caso raro
      // (admin asigna uno que ya iba en camino), no decir "recógelo".
      const message =
        status === "on_the_way"
          ? "🛵 Pedido asignado · ya va en camino"
          : "🛵 Nuevo pedido · recógelo y sal a entregar";
      toast(message, {
        id: orderId,
        duration: Infinity,
        action: {
        ...
        .on(
          ...
          (payload) => {
            ...
            if (isDriver && isActionableFor(row)) {
              notifyAssigned(row.id, row.status);
            }
        ...
        .on(
          ...
          (payload) => {
            ...
            if (isDriver && becameActionable) {
              notifyAssigned(newRow.id, newRow.status);
            }
