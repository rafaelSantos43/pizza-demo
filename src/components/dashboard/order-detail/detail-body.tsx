import { CreditCard, MapPin, Phone, Store } from "lucide-react";
...
export function DetailBody({
  ...
  const isPickup = detail.delivery_type === "pickup";
  const addressLines = isPickup ? [] : buildAddressLines(detail);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {isPickup ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-secondary px-2 py-0.5 text-[11px] font-semibold tracking-wider text-secondary-foreground uppercase">
      ...
      {isPickup ? (
        <Section title="Entrega" icon={Store}>
          <div className="flex items-center gap-2 rounded-lg border border-secondary/40 bg-secondary/15 px-4 py-3">
            <Store className="size-5 shrink-0 text-secondary-foreground" />
            <p className="font-medium text-foreground">
              El cliente recoge en el local
            </p>
          </div>
        </Section>
      ) : (
