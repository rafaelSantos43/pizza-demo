...
import type { DeliveryType, OrderStatus } from "@/features/orders/types";
...
// cliente. La hora se inserta dinámicamente arriba. El branch por
// `deliveryType` cambia el copy de `ready` y `delivered` para pickup
// (cliente recoge en el local, no hay tránsito ni mensajero).
function messageForStatus(
  ...
  const addrTag = businessAddress ? ` en ${businessAddress}` : "";
  switch (toStatus) {
    ...
    case "ready":
      return isPickup
        ? `Tu pedido${tag} está listo, ven a recogerlo${addrTag} 🍕`
        : `Tu pedido${tag} está listo, sale en minutos 🛵`;
    case "on_the_way":
      // Pickup nunca llega aquí (state machine lo previene).
      return `Tu pedido${tag} está en camino 🚗`;
    case "delivered":
      return isPickup
        ? `¡Gracias por preferirnos! Esperamos que disfrutes tu pizza 🍕`
        : `Tu pedido${tag} fue entregado ✅ ¡Gracias por preferirnos!`;
    case "payment_rejected":
      // En Twilio sandbox el camino B (responder con foto) no funciona:
      // el webhook ignora MediaUrl0. Mientras eso siga así, dirigimos
      // al cliente a contactar al restaurante; el cajero coordina por
      // este mismo chat la reenvío o cancelación. Cuando vuelva Meta
      // Cloud API, restaurar el copy de "responde a este chat con la
      // foto" (camino B) — el webhook Meta sí asocia la imagen.
      return (
        `No pudimos validar el comprobante de tu pedido${tag} 🙏 ` +
        `Contáctanos por este chat para coordinar.`
      );
    default:
...
async function getOrderForNotification(
  ...
  const { data, error } = await supabaseAdmin
    .from("orders")
    .select("created_at, delivery_type, customer:customers(phone)")
    .eq("id", orderId)
    .maybeSingle();
  if (error) {
...
async function getBusinessAddress(): Promise<string | null> {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) {
    console.error("[notifications] settings lookup failed", error);
    return null;
  }
  return (data as { business_address: string | null } | null)
...
export async function sendOrderUpdate(
  ...
  // Solo cargamos business_address si el pedido es pickup y el copy lo necesita.
  const businessAddress =
    order.delivery_type === "pickup" && toStatus === "ready"
      ? await getBusinessAddress()
      : null;
