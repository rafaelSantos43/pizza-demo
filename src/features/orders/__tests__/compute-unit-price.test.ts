...
describe("computeUnitPrice", () => {
  ...
  it("adds addonExtra on top of base price when no flavors", () => {
    const priceMap = mapOf([[`${base}:mediana`, 42000]]);
    const r = computeUnitPrice({
      baseProductId: base,
      flavors: undefined,
      size: "mediana",
    ...
    expect(r).toEqual({ ok: true, price: 49000 });
  });

  it("adds addonExtra on top of MAX(base, flavors) for multiflavor", () => {
    const priceMap = mapOf([
      [`${base}:familiar`, 63000],
      [`${f1}:familiar`, 93000],
    ]);
    ...
    expect(r).toEqual({ ok: true, price: 102000 });
  });
});
