...
export async function getSettings(): Promise<Settings> {
  ...
  const { data, error } = await supabase
    .from("settings")
    .select(
      "business_name, business_address, payment_accounts, delivery_zones, pizza_addons, pickup_prep_min",
    )
    .maybeSingle();
  ...
  const row = data as {
    ...
    pizza_addons: Settings["pizza_addons"] | null;
  ...
  return {
    ...
    pizza_addons: row.pizza_addons ?? [],
    pickup_prep_min: row.pickup_prep_min ?? 30,
  };
}
