...
import { NewManualOrderButton } from "@/components/dashboard/new-manual-order-button";
...
const valuesKey: FilterKey[] = ["all", "awaiting_payment", "preparing", "ready", "on_the_way", "delayed"]


function isFilterKey(value: string | undefined): value is FilterKey {
  return typeof value === "string" && valuesKey.includes(value as FilterKey);
}
...
export default async function PedidosPage({
  ...
  const [{ filter: filterParam }, allOrders, drivers, settings] =
    await Promise.all([
      searchParams,
      listActiveOrders(),
      listActiveDrivers(),
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      <header className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        {staff.role !== "kitchen" ? <NewManualOrderButton /> : null}
      </header>
