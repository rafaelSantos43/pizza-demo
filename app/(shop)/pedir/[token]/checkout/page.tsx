...
import { getCustomerProfile } from "@/features/orders/queries";

export const metadata: Metadata = {
  title: pageTitle("Pago"),
};
...
export default async function CheckoutPage({
  ...
  if (!verify.ok) {
    ...
    const message =
      REASON_MESSAGE[verify.reason] ?? "Este enlace no es válido.";
    return (
  ...
  const [settings, profile] = await Promise.all([
    getSettings(),
    getCustomerProfile(verify.customerId),
  ]);

  return (
    <CheckoutForm token={token} settings={settings} profile={profile} />
  );
}
